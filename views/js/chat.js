...
function modalConfirm(title, desc, submit, close){
  console.log("modalConfirm is running");
...
function modalInput(title, desc, submit, close){
  console.log("modalInput is running");
...
function getChannelList(isFirstPage) {
  console.log("getChannelList is running");
...
function createChannelList(channels) {
  console.log("createChannelList is running");
...
function joinChannel(channelUrl) {
  console.log("joinChannel is running");
...
function addChannel() {
  console.log("addChannel is running");
...
function leaveChannel(channel, obj) {
  console.log("leaveChannel is running");
...
function getMemberList(channel) {
  console.log("getMemberList is running");
 ...
 *                MESSAGING
 **********************************************/
$('#btn_messaging_chatUnl').click(function() {
  popupInit();
  $('.modal-guide-create').hide();
  $('.left-nav-button-guide').hide();
  $('.modal-open-chat').hide();
  ...
  if ($(this).hasClass('left-nav-messagingUnl--active')) {
    $('.right-section__modal-bg').hide();
    $(this).removeClass('left-nav-messagingUnl--active');
    $('.modal-messaging').hide();
  } else {
    $('.right-section__modal-bg').show();
    ...
    getUserListUnlimited(true);
    $('.modal-messaging').show();
  }
});

function getUserList(isFirstPage) {
  console.log("getUserList is running");
...
console.log("Here is the user list " + sb.createUserListQuery);
  ...
  //Grabs a random user from logged in list.
  var rand = Math.floor((Math.random() * userList.length) + 1);
  user = userList[rand];
   if (!isCurrentUser(user)) {
   //Runs startMessaging function with rand user parameter.
  startMessaging(user);
  getUserRun++;
  console.log("Running startMessaging " + getUserRun
 + " times with this user" + user);
     }

        
        if (!isCurrentUser(user)) {
          $('.modal-messaging-list').append(
            '<div class="modal-messaging-list__item" onclick="userClick($(user))">' +
            (user.nickname.length > 12 ? xssEscape(user.nickname.substring(0, 12)) + '...' : xssEscape(user.nickname)) +
            '  <div class="modal-messaging-list__icon modal-messaging-list__icon--select" data-guest-id="' + user.userId + '"></div>' +
            '</div>'
...
function userListLoadMore() {
  console.log("userListLoadMore is running");
  
  getUserList(false);
}

function userClick(obj) {
  console.log("userClick is running");

  var el = obj.find('div');
  console.log("Im a click event!");
...
function startMessaging(uid) {
  ...
  var startMessagingProcess = function(){
    console.log("startMessagingProcess is running");
    var users = [];
    
      console.log(uid);
      //Needs to = <div class="modal-messaging-list__icon modal-messaging-list__icon--select" data-guest-id="43f812eb-eb2a-40ea-bed9-1afb46996770"></div> or users needs to directly get the id's.
      users.push(uid);
      console.log(users[0]);
  ...
  var isDistinct;
  modalConfirm('Create Channel', 'You have 3 minutes to chat, then you will be switched to a new person. Start speed flirting?', function(){
    isDistinct = true;
...
/*******************************************UNLIMITED CHAT FUNCTION BEGIN**********************************************************/
function getUserListUnlimited(isFirstPage) {
console.log("getuserlistunlimited is running");
  if (isFirstPage) {
    $('.modal-messaging-list').html('');
    UserListQuery = sb.createUserListQuery();
  }
  ...
  if (UserListQuery.hasNext) {
    UserListQuery.next(function(userList, error){
      ...
      console.log("Still running 3");
          ...
          $('.modal-messaging-list').append(
            '<div class="modal-messaging-list__item" onclick="userClickUnlimited($(this))">' +
            (user.nickname.length > 12 ? xssEscape(user.nickname.substring(0, 12)) + '...' : xssEscape(user.nickname)) +
...
function userClickUnlimited(obj) {
  console.log("userClickUnlimited is running");
...
function startMessagingUnlimited() {
  if ($('.modal-messaging-list__icon--select').length == 0) {
    alert('Please select user');
    return false;
  }

  var startMessagingProcessUnlimited = function(){
    console.log("startMessagingProcessUnlimited is running");
    var users = [];
  ...
  modalConfirm('Create Channel', 'Do you want to create distinct channel?', function(){
    isDistinct = true;
    startMessagingProcessUnlimited();
  }, function(){
    isDistinct = false;
    startMessagingProcessUnlimited();
  });
...
/*******************************************UNLIMITED CHAT FUNCTION END**********************************************************/




function deleteChannel(channel) {
  console.log("deleteChannel is running");
...
function moveToTopGroupChat(channelUrl) {
  console.log("moveToTopGroupChat is running");
...
function updateGroupChannelLastMessage(message){
  console.log("updateGroupChannelLastMessage is running");
...
function updateGroupChannelListAll(){
  console.log("updateGroupChannelListAll is running");
...
function addGroupChannel(isGroup, channelMemberList, targetChannel) {
  console.log("addGroupChannel is running");
...
function groupChannelListMembersAndProfileImageUpdate(targetChannel){
  console.log("groupChannelListMembersAndProfileImageUpdate is running");
...
function joinGroupChannel(channelUrl, callback) {
  console.log("joinGroupChannel is running");
...
function endMessaging(channel, obj) {
  console.log("endMessaging is running");
  end
  popupInit();
...
function hideChannel(channel) {
  console.log("hideChannel is running");
...
function inviteMember() {
  console.log("inviteMember is running");
  ...
  var userIds = [];
  $.each($('.modal-messaging-list__icon--select'), function(index, user) {
 console.log("This is index " + user);
...
function getGroupChannelList() {
  console.log("getGroupChannelList is running");
...
function makeMemberList(members) {
  console.log("makeMemberList is running");
...
function startSendBird(userId, nickName) {
  console.log("startSendBird is running");
  ...
  console.log("initPage is running");
  ...
  console.log("onMessageReceived is running");
  ...
  console.log("SendMessageHandler is running");
...
var showTypingUser = function(channel){
  console.log("showTypingUser is running");
...
function initChatTitle(title, index) {
  console.log("initChatTitle is running");
...
var scrollPositionBottom = function() {
  console.log("scrollPositionBottom is running");
...
function loadMoreChatMessage(func) {
  console.log("loadMoreChatMessage is running");
...
function messageList(message) {
  console.log("messageList is running");
...
function updateChannelMessageCache(channel, message) {
  console.log("updateChannelMessageCache is running");
...
function updateChannelMessageCacheAll(channel) {
  console.log("updateChannelMessageCacheAll is running");
...
function fileMessageList(message) {
  console.log("fileMessageList is running");
...
function imageMessageList(obj) {
  console.log("imageMessageList is running");
...
function setWelcomeMessage(name) {
  console.log("setWelcomeMessage is running");
...
function setImageMessage(obj) {
  console.log("setImageMessage is running");

  $('.chat-canvas').append(imageMessageList(obj));
  scrollPositionBottom();
}
//Should add profile photo here
function setFileMessage(obj) {
  console.log("setFileMessage is running");
...
function setSysMessage(obj) {
  console.log("setSysMessage is running");
...
function setBroadcastMessage(obj) {
  console.log("setBroadcastMessage is running");
...
function unreadCountUpdate(channel) {
  console.log("unreadCountUpdate is running");
...
function addUnreadCount(item, unread, targetUrl) {
  console.log("addUnreadCount is running");
...
function showChannel(channel, unread, targetUrl) {
  console.log("showChannel is running");
...
function navInit() {
  console.log("navInit is running");
...
function popupInit() {
  console.log("popupInit is running");
...
window.onfocus = function() {
  console.log("onfocus is running");
